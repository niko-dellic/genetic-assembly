...
The solver adapter protocol is language-neutral JSON messages over standard input/output. The executable [`examples/python/adapter.py`](https://github.com/niko-dellic/genetic-assembly/blob/main/examples/python/adapter.py) implements v3 initialization, evaluation, finalist validation, materialization and shutdown with Python's standard library. It records each seed measurement through `/v3/evaluations`.

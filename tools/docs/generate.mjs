...
const entries = [
  ["sdk", "sdk", "quickstart"],
  ["sdk", "node", "integrating-another-repository", "src/node/index.ts"],
  ["inspector", "inspector", "results"],
  ["client", "three", "three"],
...
const index = [],
  functions = [];
for (const [directory, slug, guide, entry] of entries) {
  ...
  if (manifest.main !== "dist/index.js" || manifest.types !== "dist/index.d.ts")
    throw new Error(`Review API entry points for ${directory}`);

  const app = await Application.bootstrapWithPlugins({
    entryPoints: [resolve(root, directory, entry ?? "src/index.ts")],
    tsconfig: resolve(root, directory, "tsconfig.json"),
    plugin: ["typedoc-plugin-markdown"],
    name: manifest.name,
    out: resolve(output, slug),
    readme: "none",
    excludePrivate: true,
  ...
  functions.push(
    `## ${manifest.name}\n\n[Usage guide](../${guide}.md) · [All exports](./${slug}/index.md)\n\n` +
      sections
        .filter((s) => /^## (Functions|Classes)/.test(s))
        .join("\n")
        .replace(/^## /gm, "### ")
        .replace(/\]\(([^)]+)\)/g, `](./${slug}/$1)`),
  );
...
await writeFile(
  resolve(output, "functions.md"),
  "# All public functions and classes\n\n" + functions.join("\n"),
);

await mkdir(resolve(root, "docs/public/docs/schemas"), { recursive: true });
await cp(
  resolve(root, "sdk/schemas"),
  resolve(root, "docs/public/docs/schemas"),
  { recursive: true },
);

...
try {
  ...
  run("npx", ["--no-install", "ga", "init", "--template", "grabm"]);
  writeFileSync(
    join(directory, "ga.config.json"),
    JSON.stringify({ entry: "study.mjs", files: ["study.mjs"], project, port }),
  );
  ...
  cpSync(resolve(root,"tools/docs/tests/failure-acceptance.mjs"),join(directory,"failure.mjs"));run("node",["failure.mjs",String(port)]);
  console.log(

...
assert.equal((await api.waitJob(baseline.id, timeout())).status, "completed");
...
execFileSync(
  ...
  [
    "compose",
    "-p",
    `ga-${JSON.parse(readFileSync("ga.config.json")).project}-v2`,
    "-f",
...
execFileSync('docker',['compose','-p',`ga-${JSON.parse(readFileSync('ga.config.json')).project}-v2`,'-f','.genetic-assembly/compose.json','restart','companion'],{stdio:'inherit'});

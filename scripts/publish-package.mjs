...
if (!["sdk", "inspector", "cli", "client", "visualizations"].includes(folder))
...
if (process.env.GITHUB_REF !== `refs/tags/v${manifest.version}`)
  throw Error("Release tag must match package version");
const artifact = JSON.parse(readFileSync("artifacts/manifest.json")).find(
  (a) => a.name === manifest.name && a.version === manifest.version,
);
if (!artifact) throw Error("Missing verified artifact");
const integrity =
  "sha512-" +
  createHash("sha512")
    .update(readFileSync(`artifacts/${artifact.filename}`))
    .digest("base64");

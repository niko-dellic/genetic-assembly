...
export function createParetoSpace3d(
  ...
  function rebuild(): void {
    if (!frame || frame.dataset.objectives.length !== 3 || frame.dataset.candidates.length === 0) {
      ...
      emptyMessage(container, "The 3D objective-space view requires exactly three objectives.");
      return;
    }
    ...
    const material = new THREE.PointsMaterial({ size: 0.055, vertexColors: true });

...
test('explicit evaluator modules execute and a hanging worker can be cancelled',async()=>{
 ...
 assert.equal((await optimizer.baseline(model)).metrics.value,1.5);
 const run=optimizer.run(defineWorkerStudy({...spec,inputs:{hang:true}},factory),{populationSize:4,generations:1});
...
test('archives reopen replay bytes without model execution and reject tampering',async()=>{
 ...
 try{
 await optimizer.baseline(model);const archive=await openArchive(await optimizer.export());optimizer.dispose();

...
onMounted(async () => {
  try {
    const { createParetoScatter, createConvergenceHistory, validateDataset } =
      await import("@genetic-assembly/visualizations");
    ...
    const scatter = createParetoScatter(scatterElement.value!, {
      onSelect(id: number) {
        selected.value = id;
    ...
    const update = () => {
      scatter.update({ dataset, selection });
      history.update({ dataset, selection });
    };
    const observer = new ResizeObserver(() => {
      scatter.resize();
      history.resize();
    });
    observer.observe(scatterElement.value!);
    observer.observe(historyElement.value!);
    update();
    destroy = () => {
      observer.disconnect();
      scatter.destroy();
...
<template>
  ...
  <p aria-live="polite">
    {{
      selected === undefined
        ? "Select a point to inspect a candidate."
